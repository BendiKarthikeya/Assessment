        ...
        try:
            ...
            if books_id in self.books_dict.keys():
                if self.books_dict[books_id]["Status"] == "Available":
                    if len(self.transactions.get(books_id, [])) < 3:
                        ...
                        self.transactions.setdefault(books_id, []).append((user_id, current_date))  # Record transaction
                        
                        
                        transactions_for_book = self.transactions.get(books_id, [])
                        for user_id, issue_date in transactions_for_book:
                            print(f"Book ID: {books_id}, User ID: {user_id}, Issue Date: {issue_date}")
...
#         self.books_dict = {}
#         with open(self.list_of_books) as bk:
#             for Id, line in enumerate(bk, 101):
#                 self.books_dict[str(Id)] = {"books_title": line.strip(),
#                                              "lender_name": "",
#                                              "Issue_date": "",
#                                              "Status": "Available"}
...
#         for key, value in self.books_dict.items():
#             book_id = key
#             book_title = value["books_title"]
#             user_id = ""
#             issue_date = ""
#             status = value["Status"]

#             if key in self.transactions:
#                 user_id = self.transactions[key][0]
...
#                 print("Book title is too long!!! Title length should be 25 chars")
#                 return self.add_books()
#             elif any(new_books.lower() == book["books_title"].lower() for book in self.books_dict.values()):
#                 print("Book already exists.")
#                 return self.add_books()
#             else:
#                 with open(self.list_of_books, "a") as bk:
#                     bk.write(f"{new_books}\n")
#                 Id = str(int(max(self.books_dict)) + 1)
#                 self.books_dict[Id] = {"books_title": new_books, "lender_name": "", "Issue_date": "", "Status": "Available"}
#                 print(f"This book '{new_books}' has been added successfully !!!")
#         except Exception as e:
#             print("An error occurred:", e)
